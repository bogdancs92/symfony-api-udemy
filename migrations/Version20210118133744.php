 ...
 */
final class Version20210118133744 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, username, email, password, name, roles, password_changed_date, enabled, confirmation_token FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(255) NOT NULL COLLATE BINARY, email VARCHAR(255) NOT NULL COLLATE BINARY, password VARCHAR(255) NOT NULL COLLATE BINARY, name VARCHAR(255) NOT NULL COLLATE BINARY, roles CLOB NOT NULL COLLATE BINARY --(DC2Type:simple_array)
        ...
        $this->addSql('INSERT INTO user (id, username, email, password, name, roles, password_changed_date, enabled, confirmation_token) SELECT id, username, email, password, name, roles, password_changed_date, enabled, confirmation_token FROM __temp__user');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE INDEX IDX_9474526CF675F31B ON comment (author_id)');
        $this->addSql('CREATE INDEX IDX_9474526C4B89032C ON comment (post_id)');
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, username, email, enabled, confirmation_token, password, password_changed_date, name, roles FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL, confirmation_token VARCHAR(40) DEFAULT NULL, password VARCHAR(255) NOT NULL, password_changed_date INTEGER DEFAULT NULL, name VARCHAR(255) NOT NULL, roles CLOB NOT NULL --(DC2Type:simple_array)
        , enabled BOOLEAN DEFAULT \'FALSE\' NOT NULL)');
        $this->addSql('INSERT INTO user (id, username, email, enabled, confirmation_token, password, password_changed_date, name, roles) SELECT id, username, email, enabled, confirmation_token, password, password_changed_date, name, roles FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
    }
}
