 ...
 */
final class Version20210114134214 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, username, email, password, name, roles FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(255) NOT NULL COLLATE BINARY, email VARCHAR(255) NOT NULL COLLATE BINARY, password VARCHAR(255) NOT NULL COLLATE BINARY, name VARCHAR(255) NOT NULL COLLATE BINARY, roles CLOB NOT NULL --(DC2Type:simple_array)
        , password_changed_date INTEGER DEFAULT NULL)');
        $this->addSql('INSERT INTO user (id, username, email, password, name, roles) SELECT id, username, email, password, name, roles FROM __temp__user');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, username, email, password, name, roles FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL, password VARCHAR(255) NOT NULL, name VARCHAR(255) NOT NULL, roles CLOB DEFAULT \'ROLE_COMMENTATOR\' NOT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO user (id, username, email, password, name, roles) SELECT id, username, email, password, name, roles FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
    }
}
